...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildPrefetchUrls(allowedPages) {
    ...
    if (hasAllowedPage(allowedPages, ['Inventory', 'Lens/Product', 'Components', '/products'])) {
      urls.push('/api/products');
    }
    if (hasAllowedPage(allowedPages, ['Expenses Users', '/expenses/users'])) {
      urls.push('/api/expenses/users');
    }
    if (hasAllowedPage(allowedPages, ['Customers Center', 'Customers Center', 'User Access and Data', 'User Access', 'Customers', '/user-access'])) {
...
if (document.querySelector('.sidebar')) {
  ...
  // BRILLE ERP clean sidebar links
  ensureLink({ href: '/home', label: 'Home', icon: 'home', prepend: true });
  ensureLink({ href: '/orders/requested', label: 'BRILLE Orders', icon: 'users' });
  ensureLink({ href: '/orders/new', label: 'Create New Order', icon: 'shopping-cart' });
  ensureLink({ href: '/stocktaking', label: 'Stocktaking', icon: 'archive' });
  ensureLink({ href: '/products', label: 'Inventory', icon: 'package' });
  ensureLink({ href: '/user-access', label: 'Customers Center', icon: 'shield' });
      ...
      overlay.innerHTML = `
        <div class="app-download-modal" role="dialog" aria-modal="true" aria-labelledby="appDownloadTitle">
          ...
          <div class="app-download-head">
            ...
            <div>
              <div class="app-download-kicker">BRILLE ERP App</div>
      ...
      if (body) {
        body.innerHTML = `
          <div class="app-download-note">
            ...
            <span>This installs the current BRILLE ERP website as an app on Android or Windows. No APK/domain is required.</span>
          </div>
...
function deriveOpsShellTitle(path) {
  ...
  const map = [
    ...
    ['/orders/requested', 'BRILLE Orders'],
    ['/stocktaking', 'Stocktaking'],
    ['/products', 'Inventory'],
    ['/user-access', 'Customers Center'],
    ['/account', 'Account'],
  ];
